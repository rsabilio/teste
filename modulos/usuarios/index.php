...
//
    ...
    if ($busca) {
        ...
        if ($busca->login !== '') {
            $vt_condicoes[] = condicao_sql::montar('login', '=', $busca->login);
        }
        if ($busca->email !== '') {
            $vt_condicoes[] = condicao_sql::montar('email', '=', $busca->email);
        }
        if ($busca->cod_grupo) {
...
//
    ...
    $form->campo_text('login', 'login', $dados->login, 128, 30, 'Login');
    $form->campo_text('email', 'email', $dados->email, 128, 30, 'E-mail');
...
//
    ...
    $dados->nome = formulario::filtrar('string', $dados->nome);
    $dados->login = formulario::filtrar('string', $dados->login);
    $dados->email = formulario::filtrar('string', $dados->email);
    $dados->cod_grupo = formulario::filtrar('string', $dados->cod_grupo);
}
