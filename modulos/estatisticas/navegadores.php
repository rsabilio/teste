...
$dados_gerais->ajuda = <<<AJUDA
  <p>Esta ferramenta apresenta a quantidade de acessos realizados com cada tipo de navegador.
...
//
    ...
    if (!$dados) {
        ...
        if (!validar_dados($dados, $erros)) {
            ...
            grafico::exibir_grafico('Gr&aacute;fico de Navegadores', $CFG->wwwroot.'webservice/grafico.php', $CFG->dirroot.'webservice/grafico.php', array('id' => $id));
        }
    }
...
//
    ...
    $vt_ordem = array(1 => 'Nome do Navegador', 2 => 'Quantidade de Acessos');
...
//
    ...
    $dados_grafico->titulo       = 'Gráfico de Navegadores';
    $dados_grafico->escala       = $escala;
    $dados_grafico->valores      = $valores;
    $dados_grafico->largura      = 100 + (35 * count($escala));
    $dados_grafico->altura       = 300;
    $dados_grafico->tipo_grafico = GRAFICO_BARRA;
    $dados_grafico->nome_arquivo = 'grafico_navegadores_'.$mes_inicio.'-'.$ano_inicio.'_'.$mes_termino.'-'.$ano_termino.($versoes ? '_versao' : '');
