...
$dados_gerais->ajuda = <<<AJUDA
  <p>Esta ferramenta apresenta a quantidade de erros ocorridos em um determinado per&iacute;odo.
...
//
    ...
    if (!$dados) {
        ...
        if (!validar_dados($dados, $erros)) {
            ...
            grafico::exibir_grafico('Gr&aacute;fico de Erros', $CFG->wwwroot.'webservice/grafico.php', $CFG->dirroot.'webservice/grafico.php', array('id' => $id));
        }
    }
...
//
    ...
    $form = new formulario($action, 'form_erros');
...
//
    ...
    $dados_grafico->titulo = 'Gráfico de Erros';
    ...
    $dados_grafico->nome_arquivo   = 'grafico_erros_'.$mes_inicio.'-'.$ano_inicio.'_'.$mes_termino.'-'.$ano_termino;
