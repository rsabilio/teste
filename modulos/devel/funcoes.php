...
$memoria = number_format(16 + $total_entidades + ($total_modulos * 2), 0, '.', '');
...
//
    ...
    $arquivos = get_arquivos($CFG->dirroot, '', $_SESSION['devel']['abrir'], false);
    ...
    if ($estatisticas) {
        ...
        if (!empty($estatisticas->funcoes_invalidas)) {
...
//
function get_arquivos($dir, $dir_rel, $abertos, $ignore_doc) {
...
// Bool $ignore_doc: flag indicando que a documentacao deve ser ignorada
//
    global $CFG;
    $site = $CFG->site;
    ...
    if ($possui_diretorios || $possui_arquivos) {
        foreach ($diretorios as $diretorio) {
            ...
            if (isset($abertos[$diretorio])) {
                ...
                $valor = get_arquivos($dir.$diretorio.'/', $dir_rel.$diretorio.'/', $abertos[$diretorio], $ignore_doc);
            } else {
        ...
        foreach ($arquivos as $arquivo) {
            ...
            if (isset($abertos[$arquivo])) {
                $valor = get_funcoes_arquivo($dir.$arquivo, $dir_rel.$arquivo, $ignore_doc);
            } else {
                $valor = false;
            }
            $lista[$link] = $valor;
...
//
function get_funcoes_arquivo($arquivo, $arq_rel, $ignore_doc) {
// String $arquivo: nome completo do arquivo
// String $arq_rel: caminho relativo ao arquivo
// Bool $ignore_doc: flag indicando se a documentacao deve ser ignorada
//
    global $CFG;
    $site = $CFG->site;
    link::normalizar($site, true);
    ...
    if (!empty($obj->parametros)) {
        ...
        do {
            ...
            case T_COMMENT:
                ...
                if (!parser_simp::validar_tipo($tipo, $erro_tipo)) {
                    if (!$ignore_doc) {
                        $obj->erro = "Tipo desconhecido \"{$p->tipo}\" para o par&acirc;metro {$p->nome} ({$erro_tipo})";
                    }
                    return $obj;
                }
...
function calcular_estatisticas() {
    ...
    calcular_estatisticas_dir($CFG->dirroot, $obj, false);
    return $obj;
}
...
//
function calcular_estatisticas_dir($dir, &$obj, $ignore_doc) {
...
//
    ...
    foreach ($itens as $i) {
        if ($i == '.' || $i == '..' || $i == '.svn') { continue; }
        if (is_file($dir.$i) && preg_match('/\.php$/', $i)) {
            contar_funcoes_arquivo($dir.$i, $obj, $ignore_doc);
        } elseif (is_dir($dir.$i)) {
            calcular_estatisticas_dir($dir.$i.'/', $obj, $ignore_doc);
        }
    }
}
...
//
function contar_funcoes_arquivo($arq, &$obj, $ignore_doc) {
...
//
    ...
    foreach ($funcoes as $f) {
        $obj->total_funcoes++;
        if (!$ignore_doc && isset($f->erro)) {
            $obj->funcoes_invalidas[$arq][$f->nome] = $f->erro.' (linha: '.$f->linha.')';
        }
    }
}
