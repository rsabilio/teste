...
//
    ...
    $vt_tipos = array(
        ...
        'Entidades da Aplica&ccedil;&atilde;o' => array(
            'entidades' => 'Todas'
        )
    );

    $padrao = array('tipo' => 'suporte');
...
//
    ...
    echo '<p>'.icone::img('importar', 'hierdado').' = m&eacute;todo herdado</p>';
}
...
//
    ...
    $eh_entidade = $rc->isSubclassOf('objeto');
...
//
    ...
    if ($count_metodos) {
        ...
        foreach ($metodos as $m) {
            ...
            // Obter o icone para importado ou nao
            if ($m->getFileName() == $rc->getFileName()) {
                $img_importado = '';
            } else {
                $img_importado = ' '.icone::img('importar');
            }
            ...
            echo "<li>\n";
            $class = ($m->getDeclaringClass()->getName() == $rc->getName()) ? 'destaque' : '';
            link::texto($link, $img.$img_escrever.$img_importado.' '.$tipo.$m->getName(), false, false, $class, false, true, false);
...
//
    ...
    $obj = objeto::get_objeto($atributo->classe);
    $desc = $atributo->descricao ? $atributo->descricao : 'valor padr&atilde;o ("'.$obj->get_entidade(true).'")';
    $chave_fk = $atributo->chave_fk;
    $index = $atributo->index ? $atributo->index : 'nenhum';
    $impressao = $atributo->impressao ? $atributo->impressao : 'valor padr&atilde;o';

    echo "<div>\n";
    echo "<p><strong>Descri&ccedil;&atilde;o</strong> {$desc}</p>\n";
    echo "<p><strong>Chave Relacionada:</strong> {$chave_fk}</p>\n";
    echo "<p><strong>Campo de Indexa&ccedil;&atilde;o:</strong> {$index}</p>\n";
    echo "<p><strong>Campo de Impress&atilde;o:</strong> {$impressao}</p>\n";
