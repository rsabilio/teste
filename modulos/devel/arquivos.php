...
function imprimir_arquivos() {
    ...
    // Exibir estatisticas
    if ($estatisticas) {
        $num_paginas = round($estatisticas->tamanho_total / 3072); // uma pagina tem aproximadamente 3KiB
        ...
        if (count($estatisticas->sistemas) > 1) {
            ...
            echo '<td>'.memoria::formatar_bytes($estatisticas->tamanho_total, true).' (100%)</td>';
            echo '<td>'.memoria::formatar_bytes($estatisticas->tamanho_real, true).' (100%)</td>';
            echo '</tr>';
            echo '</tfoot>';
            echo '<tbody>';
            ...
            foreach ($estatisticas->sistemas as $sistema) {
                ...
                echo '<td>'.memoria::formatar_bytes($sistema->tamanho_total, true).' ('.$sistema->porcentagem->tamanho_total.'%)</td>';
                echo '<td>'.memoria::formatar_bytes($sistema->tamanho_real, true).' ('.$sistema->porcentagem->tamanho_real.'%)</td>';
                echo '</tr>';
            }
            echo '</tbody>';
            echo '</table>';
            ...
            echo '<p><em>Tamanho dos Arquivos PHP:</em> '.memoria::formatar_bytes($estatisticas->tamanho_total, true);
            ...
            echo '<p><em>Tamanho dos Arquivos PHP:</em> '.memoria::formatar_bytes($estatisticas->tamanho_total, true);
            echo ' (corresponde a um livro de '.$num_paginas.' p&aacute;ginas A4)</p>';
            echo '<p><em>Tamanho dos Arquivos PHP sem coment&aacute;rios e espa&ccedil;os:</em> '.memoria::formatar_bytes($estatisticas->tamanho_real, true).'</p>';
            echo '<p>'.$porcentagem.'% do c&oacute;digo s&atilde;o coment&aacute;rios e espa&ccedil;os: '.$situacao.'</p>';
        }
        ...
        $dados_grafico->legenda = array('Autor Principal', 'Autor Secundário');
        foreach ($estatisticas->autores as $autor => $dados) {
            echo '<tr>';
            echo '<td scope="row">'.$autor.'</td>';
            echo '<td>'.$dados->quantidade.'</td>';
            echo '<td>'.memoria::formatar_bytes($dados->tamanho, true).'</td>';
            if (isset($estatisticas->secundarios[$autor])) {
                ...
                echo '<td>'.memoria::formatar_bytes($dados_secundario->tamanho, true).'</td>';
            } else {
            ...
            $dados_grafico->valores[0][] = $dados->tamanho;
            $dados_grafico->valores[1][] = $dados_secundario->tamanho;

            echo '<td>'.($total_quantidade).'</td>';
            echo '<td>'.memoria::formatar_bytes($total_tamanho, true).'</td>';
        ...
        grafico::exibir_grafico('Atuação dos Autores', $CFG->wwwmods.'devel/grafico_pilha.php', $CFG->dirmods.'devel/grafico_pilha.php', $dados_grafico);

        if (!empty($estatisticas->arq_invalidos)) {
...
//
    ...
    $possui_diretorios = !empty($diretorios);
    $possui_arquivos = !empty($arquivos);
    $lista = array();
    if ($possui_diretorios || $possui_arquivos) {
...
//
    global $CFG;
    if (!preg_match('/\.([a-z]+)$/i', $arquivo, $match)) {
...
//
    ...
    $itens = scandir($dir);
    if (!$ignore_doc && in_array('.ignoredoc', $itens)) {
        $ignore_doc = true;
    }
    if ($ignore_doc) {
        return;
