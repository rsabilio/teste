...
// Pagina para onde o usuario e' redirecionado caso nao esteja logado
$link_deslogado = $CFG->wwwlogin;
...
if (!isset($_SESSION[$CFG->codigo_session])) {
    ...
    if ($CFG->site != $CFG->wwwroot &&
        ...
        $link = link::adicionar_atributo($link_deslogado, 'destino', $destino);
...
if (objeto::em_cache('arquivo', 1)) {
    $arquivo_principal = objeto::get_cache('arquivo', 1);
} else {
    $campos_arquivo = array(
        'arquivo',
        'modulo',
        'descricao'
    );
...
unset($arq, $v, $c, $nav, $modulo);

// Verificar se user_agent nao mudou
if ($_SESSION['md5_user_agent'] != md5($_SERVER['HTTP_USER_AGENT'])) {
