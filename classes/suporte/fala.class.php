...
define('FALA_CONVERSOR_MP3',  '/usr/bin/lame ${entrada} ${saida} &> /dev/null');
...
final class fala {
    ...
    //
        ...
        $obj = '<object class="fala" type="application/x-shockwave-flash" width="'.$largura.'" height="'.$altura.'" data="'.$link.'">'.
    ...
    //
        ...
        $opcoes_http = array(
            'arquivo' => $arquivo.'.mp3',
            'tempo_expira' => 31536000,
            'compactacao' => true,
            'ultima_mudanca' => filemtime($CFG->dirarquivos.'simp_fala/'.$arquivo)
        );

        http::cabecalho('audio/mpeg', $opcoes_http);
        echo $conteudo;
        exit(0);
    }
