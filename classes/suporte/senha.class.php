...
final class senha {
    ...
    //
    //     Gera uma sequencia com valores dos intervalos informados
    //
    public static function gerar_com_intervalo($tamanho, $intervalos) {
    // Int $tamanho: tamanho da sequencia
    // Array[Array[Int]] $intervalos: vetor de vetores com intervalo minimo e maximo (na forma de ord)
    //
        $saida = '';
        for ($i = $tamanho; $i > 0; --$i) {
            $saida .= self::get_rand_char_intervalo($intervalos);
        }
        return $saida;
    }
    ...
    //
    //     Obtem um caractere dentro de um dos intervalos
    //
    public static function get_rand_char_intervalo($intervalos) {
    // Array[Array[Int]] $intervalos: vetor de vetores com intervalo minimo e maximo (na forma de ord)
    //
        $total = 0;
        foreach ($intervalos as $intervalo) {
            list($inicio, $termino) = $intervalo;
            $total += ($termino - $inicio);
        }
        $r1 = mt_rand(0, $total - 1);

        $total = 0;
        foreach ($intervalos as $intervalo) {
            list($inicio, $termino) = $intervalo;
            if ($r1 >= $total && $r1 <= ($total + ($termino - $inicio))) {
                break;
            }
            $total += ($termino - $inicio);
        }

        $r2 = mt_rand($inicio, $termino);
        return chr($r2);
    }
