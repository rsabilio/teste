...
final class instalacao {
    private static $bd_config;                 // Dados para conexao com o BD para instalacao
    ...
    //
    public static function set_bd_config($bd_config) {
    ...
    //
    public static function checar_pre_requisitos() {
        $erro = false;

        // Verificar a versao do PHP
        if (!self::validar_php($erro)) {
            pagina::erro(null, $erro, $erro);
            exit(1);

        // Verificar os modulos do PHP instalados
        } elseif (!self::checar_modulos($vt_modulos)) {
        ...
        } elseif (self::possui_erros_classes($classes_erradas)) {
        ...
        } elseif (self::possui_erros_ini($erros)) {
    ...
    //
    public static function validar_php(&$erro) {
    ...
    //
    public static function checar_modulos(&$vt_modulos) {
    ...
    //
        ...
        foreach ($vt_importancia as $modulo => $importancia) {
            ...
            $obj->situacao = ($obj->importancia == 6 && $obj->carregado) || ($obj->importancia != 6);
    ...
    //
    public static function possui_erros_ini(&$erros) {
    // Array[String] $erros: vetor de erros encontrados
    //
        // @ MEMORY_LIMIT
        $memory_limit = ini_get('memory_limit');
        if (intval($memory_limit) == -1) {
            // Sem limite de memoria
        } else {
            $memory_limit_bytes = memoria::desformatar_bytes_php($memory_limit);

            // Abaixo de 8M
            if ($memory_limit_bytes < (8 * pow(2, 10))) {
    ...
    //
    public static function imprimir_licenca() {
...
LICENCA;
    ...
    //
    public static function formulario_instalacao(&$dados) {
    // Object $dados: dados enviados pelo formulario
    //
        global $CFG;

        $vt_charset = array(
            'utf-8'      => 'Unicode [UTF-8] (Padr&atilde;o)',
        ...
        foreach ($vt_driver as $driver) {
            ...
            if (!$sgbd_padrao && !$porta_padrao) {
                $sgbd_padrao = $driver->codigo;
                $porta_padrao = $driver->porta;
            }
        }
        $vt_portas[0] = 'Outra (especificar abaixo)';

        $vt_usar_bd = array(
            0 => 'Criar Nova Base de Dados',
            1 => 'Usar Base de Dados'
        );

        $vt_usar_usuario = array(
            0 => 'Criar Novo Usu&aacute;rio',
...
AJUDA;
    ...
    //
    public static function possui_erros(&$dados, &$erros) {
    ...
    //
    public static function criar_bd(&$dados, &$erros, &$avisos) {
    // Object $dados: dados submetidos
    ...
    //
    public static function criar_tabelas($tabelas, &$erros, &$avisos, &$resultado, $charset = 'utf-8') {
    ...
    //
        ...
        case INSTALACAO_TODAS_TABELAS:
            $vt_objetos = self::get_objetos();
            break;

        case INSTALACAO_TABELAS_NECESSARIAS:

            // Percorrer as classes em busca de tabelas nao criadas
            $entidades = listas::get_entidades(false);
            foreach ($entidades as $nome_classe => $entidade) {
                if (simp_autoload($nome_classe)) {
                    $obj = objeto::get_objeto($nome_classe);
                    $nome_tabela = $obj->get_tabela();
                    if (!in_array($nome_tabela, $nomes_tabelas)) {
                        $vt_objetos[] = $obj;
                    }
                } else {
                    $erros[] = 'A classe '.$nome_classe.' n&atilde;o foi encontrada ou possui erros';
                }
            }
            break;
        ...
        foreach ($vt_objetos as $obj) {
            $sql = $bd->sql_comment('TABELA '.$obj->get_tabela())."\n".
                   $bd->formatar_sql($bd->sql_create_table($obj, $charset));
            $resultado .= '<div>'.
                          "<pre>{$cdata}{$sql}{$fcdata}</pre>";
    ...
    //
    public static function remover_tabelas($entidades, &$erros) {
    ...
    //
    public static function criar_usuario(&$dados, &$erros, &$avisos) {
    // Object $dados: dados submetidos
    // Array[String] $erros: erros ocorridos
    // Array[String] $avisos: avisos ocorridos
    //
    ...
    //
    public static function criar_arquivo($dados, &$erros, &$avisos) {
    // Object $dados: dados submetidos
    // Array[String] $erros: erros ocorridos
    // Array[String] $avisos: avisos ocorridos
    //
...
require(\$dirroot.'var.php'); // Nao retirar esta linha!!!
...
ARQ;
    ...
    //
    public static function formulario_instalacao_classes(&$dados) {
    // Object $dados: dados submetidos
    //
    ...
    //
    public static function instalar_classes(&$erros, &$avisos) {
    // Array[String] $erros: erros ocorridos
    // Array[String] $avisos: avisos ocorridos
    //
        global $CFG;
        ...
        foreach (listas::get_entidades(false) as $classe => $descricao) {
            $obj = objeto::get_objeto($classe);
            $bd->truncate($obj);
        }
        unset($obj, $bd);
        ...
        $funcoes = self::get_funcoes_instalacao($erros);

        // Ordenar funcoes pelas dependencias
        if (self::possui_loop_dependencias($funcoes, $erros)) {
            return false;
        }
        $funcoes = self::ordenar_funcoes($funcoes);
        ...
        foreach ($funcoes as $dados) {
            ...
            // Limpar cache para garantir que nao vai buscar em sessao
    ...
    //
    //     Obtem a lista de funcoes para instalacao
    //
    public static function get_funcoes_instalacao(&$erros) {
    // Array[String] $erros: erros ocorridos
    //
        global $CFG;
        $funcoes = array();
        ...
        while (($arq = readdir($dir)) !== false) {
            if (!preg_match('/^([a-z0-9-_]+)\.php$/i', $arq, $matches)) {
                continue;
            }
            $classe = $matches[1];
            unset($matches);
            ...
            if (function_exists($funcao)) {
                ...
                $dados->funcao = $funcao;
                $dados->classe = $classe;
                $dados->dependencias = function_exists($funcao_dep) ? $funcao_dep() : array();
    ...
    //
    public static function possui_loop_dependencias($funcoes, &$erros) {
    ...
    //
        ...
        foreach ($funcoes as $classe => $dados) {
            $vetor = array();
            $loop  = '';
            if (self::possui_loop_dependencias_rec($dados, $funcoes, $vetor, $loop)) {
    ...
    //
    private static function possui_loop_dependencias_rec($dados, $funcoes, $vetor, &$loop) {
    ...
    //
        ...
        foreach ($dados->dependencias as $classe_dependente) {
            ...
            if (self::possui_loop_dependencias_rec($dados2, $funcoes, $vetor2, $loop)) {
                return true;
            }
        }
        return false;
    ...
    //
    public static function adicionar_atributo($classe, $atributo, &$erro) {
    ...
    //
        if (simp_autoload($classe)) {
            ...
            $erro = 'A classe '.$classe.' n&atilde;o foi encontrada ou possui erros';
            return false;
        }
    ...
    //
    public static function remover_atributo($classe, $atributo, &$erro) {
    // String $classe: nome da classe
    // String $atributo: nome do atributo
    // String $erro: erro ocorrido
    //
    ...
    //
    public static function get_objetos() {
        ...
        foreach ($classes as $classe) {
            $objetos[$classe] = objeto::get_objeto($classe);
        }

        // Ordenar os objetos de acordo com as dependencias
        $objetos = array_values($objetos);
        $objetos_ordenados = self::ordenar_objetos($objetos);
    ...
    //
    private static function ordenar_funcoes(&$funcoes) {
    // Array[Object] $funcoes: vetor de dados das funcoes
    //
        $retorno = array();
        $classes = array();
        foreach ($funcoes as $classe => $funcao) {
            self::inserir_funcao($retorno, $classes, $funcao, $funcoes);
        }
        return $retorno;
    }
    ...
    //
    private static function inserir_funcao(&$vetor, &$classes, $funcao, $funcoes) {
    ...
    //
        foreach ($funcao->dependencias as $classe_dependente) {
            if (isset($funcoes[$classe_dependente])) {
                ...
                self::inserir_funcao($vetor, $classes, $funcao_dependente, $funcoes);
            }
        }
    ...
    //
    private static function ordenar_objetos(&$objetos) {
    // Array[Object] $objetos: vetor de objetos
    //
        $retorno = array();
        foreach ($objetos as $obj) {
            self::inserir_objeto($retorno, $obj, $objetos);
        }
        return $retorno;
    }
    ...
    //
    private static function inserir_objeto(&$vetor, $obj, $objetos) {
    ...
    //
        ...
        foreach ($dependencias as $classe_dependente) {
            if (!self::inserido($classe_dependente, $vetor)) {
                $obj_dependente = self::get_objeto($classe_dependente, $objetos);
                if (!$obj_dependente) {
                    trigger_error('Classe desconhecida: '.$classe_dependente, E_USER_ERROR);
                    continue;
                }
                self::inserir_objeto($vetor, $obj_dependente, $objetos);
            }
        }
        unset($dependencias);
        if (!self::inserido($obj->get_classe(), $vetor)) {
            $vetor[] = $obj;
        }
    }
    ...
    //
    private static function get_objeto(&$classe, &$objetos) {
    ...
    //
    private static function inserido($classe, &$vetor) {
    // String $classe: nome da classe
    ...
    //
    private static function get_arquivos_classes($diretorio, &$classes) {
    ...
    //
        ...
        while (($arq = readdir($dir)) !== false) {
            ...
            if (is_dir($diretorio.'/'.$arq)) {
                self::get_arquivos_classes($diretorio.'/'.$arq.'/', $classes);
            } elseif (preg_match('/^([a-z0-9-_]+)\.class\.php$/i', $arq, $vetor)) {
                $classe = $vetor[1];
                unset($vetor);
    ...
    //
    public static function possui_erros_classes(&$classes_erradas) {
    ...
    //
        ...
        self::get_arquivos_classes(INSTALACAO_DIR_CLASSES, $classes);
        ...
        // Guardar vetor de funcoes e dependencias
        while (($arq = readdir($dir)) !== false) {
            if (!preg_match('/^([a-z0-9-_]+)\.php$/i', $arq, $vetor)) {
                continue;
            }
            $classe = $vetor[1];
            unset($vetor);
