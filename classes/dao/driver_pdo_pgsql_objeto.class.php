...
final class driver_pdo_pgsql_objeto extends driver_objeto {
    ...
    // String $funcao: nome da funcao (dia, mes, ano, hora, minuto, segundo, diaano, diasemana, data, horario)
    ...
    //
        ...
        if (isset($funcoes_extract[$funcao])) {
            switch ($tipo_operando) {
            case CONDICAO_SQL_TIPO_ATRIBUTO:
                return $this->delimitar_funcao('extract').'('.$funcoes_extract[$funcao].' from '.$operando.')';
            case CONDICAO_SQL_TIPO_VALOR:
                return $this->delimitar_funcao('extract').'('.$funcoes_extract[$funcao].' from TIMESTAMP '.$operando.')';
            }
        } else {
            switch ($funcao) {
            case 'data':
        ...
        trigger_error('Driver PDO PostgreSQL nao suporta a funcao "'.$funcao.'"', E_USER_WARNING);
    }
    ...
    //
        ...
        case 'data':
            return sprintf(
                '%04d-%02d-%02d',
                isset($vt_data[2]) ? max((int)$vt_data[2], 1) : 1,
