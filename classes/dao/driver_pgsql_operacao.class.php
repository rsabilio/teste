...
/// @ METODOS DE OPERACOES SOBRE O BANCO DE DADOS
    ...
    public function get_tabelas() {
        ...
        $sql = 'SELECT c.relname AS tabela, obj_description(c.oid, \'pg_class\') AS comentario, '.
               'pg_total_relation_size(cast(c.relname as varchar)) as tamanho, c.* '.
               'FROM pg_class c LEFT JOIN pg_namespace n ON (n.oid = c.relnamespace) '.
               'WHERE c.relkind = \'r\' AND nspname = \'public\' '.
               'ORDER BY c.relname';

        $resultado = $this->consultar($sql);
        if ($resultado && $this->quantidade_registros($resultado)) {
    ...
    //
        ...
        if ($resultado) {
            ...
            while ($obj = $this->fetch_object($result3)) {
                if (stripos($obj->padrao, 'nextval') !== false) {
            ...
            foreach ($resultado as $nome => $atributos) {
                ...
                $tabela_ref   = isset($extras[$nome]->tabela_ref) ? $extras[$nome]->tabela_ref : null;
                $atributo_ref = isset($extras[$nome]->atributo_ref) ? $extras[$nome]->atributo_ref : null;
    ...
    //
        ...
        $sql_usuario = $this->delimitar_valor($usuario, 'string');
