...
/// @ METODOS ESPECIFICOS DA CLASSE
    ...
    //
    public function filtrar_atributo_php($atributo, $valor) {
    // atributo $atributo: definicao do atributo
    // Mixed $valor: valor do atributo
    //
        if (is_null($valor)) {
        ...
        case 'binario':
            ...
            $retorno = $this->delimitar_valor($retorno, $atributo->tipo);
            break;
    ...
    //
        ...
        $vetor = array();
        foreach ($vetor_agrupado as $grupo => $funcoes) {
            $vetor = array_merge($vetor, $funcoes);
        }
        return $vetor;
    }
    ...
    //
        ...
        case 'diaano':
            ...
            break;
        case 'diasemana':
            $atributo = new atributo('diasemana', 'Dia da semana', 0);
            $atributo->set_tipo('int', false);
            $atributo->set_intervalo(1, 7);
            break;
        case 'data':
            $atributo = new atributo('data', 'Data', '00-00-0000');
            $atributo->set_tipo('data', false);
            $atributo->set_campo_formulario('data');
            break;
        case 'horario':
            $atributo = new atributo('horario', 'Hor&aacute;rio', '00-00-0000-00-00-00');
            $atributo->set_tipo('data', false);
            $atributo->set_campo_formulario('hora');
            break;
        default:
            return false;
        }
...
/// @ METODOS DE GERACAO DE SQL NO PADRAO SQL92 (DEVEM SER SOBRECARREGADOS CASO NECESSARIO)
    ...
    //
    //     COMMENT: gera uma SQL com um comentario
    //
    public function sql_comment($texto) {
    ...
    //
        ...
        foreach ($estrutura_consulta as $dado) {
            ...
            $sql_agrupamento  = $this->gerar_sql_agrupamento($objeto, $dado, $dado->precisa_usar_apelidos_tabelas());
    ...
    // condicao_sql $condicoes: condicoes da consulta
    //
        return $this->sql_funcao($objeto, 'MAX', $atributo, $apelido, $condicoes, false);
    }
    ...
    // condicao_sql $condicoes: condicoes da consulta
    //
        return $this->sql_funcao($objeto, 'MIN', $atributo, $apelido, $condicoes, false);
    }
    ...
    //
        ...
        return $this->sql_funcao($objeto, 'COUNT', $atributo, $apelido, $condicoes, true);
    }
    ...
    // condicao_sql $condicoes: condicoes da consulta
    //
        return $this->sql_funcao($objeto, 'SUM', $atributo, $apelido, $condicoes, false);
    }
        ...
        foreach ($dados as $chave => $valor) {
            ...
            if ($objeto->possui_rel_uu($chave, false)) {
                ...
                if (!$def_rel->forte) {
                    ...
                    $valor = $this->filtrar_atributo_php($def, $valor ? $valor : null);
            ...
            $vt_sql_campos[] = $this->delimitar_campo($chave);
            $def = $objeto->get_definicao_atributo($chave);
            $valor = $this->filtrar_atributo_php($def, $valor);
            $vt_sql_valores[] = $valor;
            $possui_campo = true;
        }
        if (!$possui_campo) {
    ...
    //
        ...
        foreach ($dados as $chave => $valor) {
            ...
            if ($objeto->possui_rel_uu($chave, false)) {
                ...
                if (!$def_rel->forte) {
                    ...
                    if (strpos($valor, 'sql:') === 0) {
                        ...
                        $def = $objeto->get_definicao_atributo($chave);
                        $sql_valor = $this->filtrar_atributo_php($def, $valor ? $valor : null);
                    }
            ...
            if (strpos($valor, 'sql:') === 0) {
                ...
                $def = $objeto->get_definicao_atributo($chave);
                $sql_valor = $this->filtrar_atributo_php($def, $valor);
            }
            $vt_sql_atribuicoes[] = $sql_chave.' = '.$sql_valor;
            $possui_campo = true;
...
/// @ METODOS AUXILIARES PARA GERACAO DE SQL
    ...
    //
    //     Obtem os nomes das constantes com os respectivos valores
    //
    public function get_constantes() {
        $nomes = array(
            'DOMINGO',
            'SEGUNDA',
            'TERCA',
        ...
        foreach ($nomes as $nome) {
            $vetor[$nome] = $this->get_constante($nome);
        }
        return $vetor;
    }
    ...
    //
    //     Obtem uma constante especial do driver
    //
    public function get_constante($nome) {
    // String $nome: nome da constante
    //
        switch ($nome) {
        case 'DOMINGO':
            return 1;
        ...
        case CONDICAO_SQL_SIMPLES:
            ...
            if ($tipo2 == CONDICAO_SQL_TIPO_ATRIBUTO) {
                ...
                if ($funcao2) {
                    ...
                    $sql_operando2 = $this->filtrar_atributo_php($def_parametro2, $condicao->operando2);
                    $sql_operando2 = $this->montar_funcao_condicao($funcao2, $sql_operando2, $tipo2, $def_parametro2);
                } else {
                    $sql_operando2 = $this->filtrar_atributo_php($def_retorno1, $condicao->operando2);
                }
            }
    ...
    //
    protected function gerar_sql_agrupamento($objeto, $dados, $apelido) {
    // objeto $objeto: objeto envolvido na consulta
    // consulta $dados: dados estrutrados da consulta
    // Bool $apelido: usar apelido do campo/tabela
    //
        $tabela_campo = $dados->get_tabela_atributo($objeto->get_chave(), $objeto->get_classe(), $apelido);
        if (!$tabela_campo) {
            return false;
        }
    ...
    //
    protected function sql_funcao($objeto, $funcao, $atributo, $apelido = null, $condicoes = null, $distinct = false) {
    // Object $objeto: instancia de uma entidade derivada da classe objeto
    ...
    // condicao_sql $condicoes: condicoes da consulta
    // Bool $distinct: aplica um distinct para o parametro da funcao
    //
        self::filtrar_condicao($condicoes);
        ...
        $usar_apelido = $dados->precisa_usar_apelidos_tabelas();
        $sql_funcao_campo = $this->gerar_sql_funcao_campo($funcao, $dados_campo, $usar_apelido, $distinct);
        $sql_tabelas      = $this->gerar_sql_tabelas($objeto, $dados);
        $sql_condicoes    = $this->gerar_sql_condicao($objeto, $dados, $usar_apelido);
    ...
    //
    protected function gerar_sql_funcao_campo($funcao, $dados, $usar_apelido_tabela = true, $distinct = false) {
    ...
    // Bool $distinct: aplicar o distinct na funcao
    //
    ...
    //
        ...
        $valor = $this->filtrar_atributo_php($atributo, $atributo->padrao);
        switch ($atributo->tipo) {
        case 'bool':
        case 'int':
        case 'float':
