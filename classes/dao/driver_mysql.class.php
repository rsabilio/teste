...
class driver_mysql extends driver_base {
    ...
    // String $tipo: tipo de valor no Simp
    //
        switch ($tipo) {
        case 'string':
        case 'data':
    ...
    //
        ...
        if (!$selecionou_base) {
            $this->get_ultimo_erro($str_erro, $cod_erro);
            $erro = "Erro ao selecionar base (Erro {$cod_erro}: {$str_erro})";
            trigger_error($erro, E_USER_WARNING);
            $this->adicionar_erro($erro);
            $this->desconectar();
            return false;
        ...
        if ($charset && $charset != mysql_client_encoding($this->conexao)) {
            if (function_exists('mysql_set_charset')) {
                ...
                $this->consultar('SET NAMES '.$charset, $this->conexao);
            }
        }
    ...
    //
        if (is_resource($resultado)) {
            if (mysql_num_rows($resultado)) {
                return mysql_data_seek($resultado, 0);
            }
            return true;
        }
