...
final class driver_mysql_objeto extends driver_objeto {
    ...
    //
        ...
        foreach ($objeto->get_atributos() as $def_atributo) {
            ...
            if ($def_atributo->chave != 'PK') {
                ...
                if ($def_atributo->unico) {
                    ...
                    $vt_constraint[$sql_constraint] = "  CONSTRAINT {$sql_constraint} UNIQUE KEY USING BTREE ({$sql_campo})";

                // Checar se e' uma chave candidata
                } elseif ($def_atributo->chave == 'CK') {
                    $vt_constraint['index_'.$def_atributo->nome] = "  INDEX USING BTREE ({$sql_campo})";
                }
            ...
            // Se e' a chave primaria
                ...
                $vt_constraint[$sql_constraint] = "  CONSTRAINT {$sql_constraint} PRIMARY KEY USING BTREE ({$sql_campo})";
            }
        }
        ...
        foreach ($objeto->get_definicoes_rel_uu() as $atributo_rel => $def_atributo_rel) {
            ...
            $vt_constraint['index_'.$def_atributo_rel->nome] = "  INDEX ({$sql_atributo_rel}) USING BTREE";
        ...
        foreach ($objeto->get_chaves_unicas_compostas() as $chave_uk) {
            ...
            $vt_constraint[$sql_constraint] = "  CONSTRAINT {$sql_constraint} UNIQUE KEY USING BTREE ({$sql_campo_uk})";
        }
    ...
    //
        ...
        // Montar SQL
        $sql = "DROP TABLE IF EXISTS {$sql_tabela}";
        return $sql;
    }
        ...
        case DRIVER_OBJETO_ADICIONAR_ATRIBUTO:
            ...
            if ($objeto->possui_rel_uu($atributo, false)) {
                ...
                if ($def_relacionamento->forte && !$objeto->possui_registros()) {
                    $sql_index = "ALTER TABLE {$sql_tabela} ADD INDEX ($sql_atributo) USING BTREE";
