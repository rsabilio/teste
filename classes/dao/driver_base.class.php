...
/// @ METODOS EXIGIDOS PELAS CLASSES FILHAS
    ...
    //     Delimita um valor
    //
    abstract public function delimitar_valor($valor, $tipo = 'string');
    // String $valor: valor qualquer
    // String $tipo: tipo de atributo do Simp
    //
    ...
    // Mixed $resultado: resultado de uma consulta
    //


    //
    //     Volta o ponteiro para a primeira posicao
    //
    abstract public function rewind($resultado);
    // Mixed $resultado: resultado de uma consulta
    //


    //
    //     Obtem o numero de resultados de um resultado
    //
    ...
    // Mixed $resultado: resultado a ser analisado
    //
    ...
    // Mixed $resultado: resultado a ser apagado
    //
...
/// @ METODOS ESPECIFICOS DA CLASSE
    ...
    final public function __destruct() {
        if ($this->transacao) {
            trigger_error('Transacao nao fechada ao desalocar objeto de acesso ao BD', E_USER_NOTICE);
            $this->fim_transacao();
        }
        $this->desconectar();
    }
    ...
    //
    //     Obtem o delimitador de instrucoes
    //
    public static function get_delimitador_instrucoes() {
        return ';';
    }
...
//file_put_contents('/tmp/sql', $_SERVER['REQUEST_URI'].': '.$sql.";\n", FILE_APPEND);
...
//}
    ...
    //
        if (is_string($sql)) {
            $di = $this->get_delimitador_instrucoes();
            return $sql.(($sql[strlen($sql) - 1] == $di) ? '' : $di);
        } elseif (is_array($sql)) {
            $vt_retorno = array();
            foreach ($sql as $s) {
