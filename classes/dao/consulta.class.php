...
/// @ METODOS AUXILIARES
    ...
    //
        ...
        foreach ($caminho as $filho) {
            ...
            if ($obj1->possui_rel_uu($filho)) {
                $def_filho = $obj1->get_definicao_rel_uu($filho);
                $obj2 = objeto::get_objeto($def_filho->classe);
                $tipo_juncao = $this->adicionar_join_objeto($obj1, $obj2, $filho, $caminho_relativo, $usou_left_join ? 'LEFT' : false);
                if ($tipo_juncao == 'LEFT') {
                    $usou_left_join = true;
                }
                unset($def_filho, $obj1);
                $obj1 = &$obj2;
    ...
    //
        ...
        $condicao->flags     = array(
            'tipo1' => CONDICAO_SQL_TIPO_ATRIBUTO,
    ...
    //
        foreach ($ordem as $campo => $tipo_ordem) {
            ...
            if ($pos !== false) {
                ...
                $def_filho = $obj->get_definicao_rel_uu($atributo_obj);
                $obj_filho = objeto::get_objeto($def_filho->classe);
            } else {
                $atributo_obj = '';
                $obj_filho = $obj;
            }
    ...
    //
    //     Obtem o nome da tabela e do campo da consulta de um determinado atributo de uma classe
    //
    public function get_tabela_atributo($atributo, $classe, $apelido = false) {
    // String $atributo: nome do atributo
    // String $classe: nome da classe
    // Bool $apelido: retorna o apelido da tabela e do campo ou os nomes reais
    //
        foreach ($this->campos as $campo) {
            if (strcmp($campo->atributo, $atributo) == 0 && strcmp($campo->tabela->classe, $classe) == 0) {
                if ($apelido) {
                    return array($campo->tabela->apelido, $campo->nome);
                } else {
                    return array($campo->tabela->nome, $campo->nome);
                }
            }
        }
        return false;
