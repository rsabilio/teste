...
class driver_pdo_mysql extends driver_base {
    ...
    public static function get_delimitador_instrucoes() {
        return self::DELIMITADOR_INSTRUCOES;
    }
    ...
    //     Delimita um valor
    //
    public function delimitar_valor($valor, $tipo = 'string') {
    ...
    public function get_versao() {
        ...
        $this->versao = $this->conexao->getAttribute(PDO::ATTR_SERVER_VERSION);
        if ($conectou) {
            $this->desconectar();
        }
        return $this->versao;
    ...
    //
        ...
        if ($charset) {
            $vt_dsn[] = 'charset='.$charset;
        }
        $dsn = 'mysql:'.implode(';', $vt_dsn);
        $opcoes = array(PDO::ATTR_PERSISTENT => $persistente);
        if ($charset) {
            $opcoes[PDO::MYSQL_ATTR_INIT_COMMAND] = 'SET NAMES '.$charset;
        }
    ...
    //
    public function rewind($resultado) {
