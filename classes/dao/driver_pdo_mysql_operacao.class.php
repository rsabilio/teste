...
/// @ METODOS DE OPERACOES SOBRE O BANCO DE DADOS
    ...
    public function get_tabelas() {
        ...
        $sql = "SHOW TABLE STATUS FROM {$this->base}";
        $resultado = $this->consultar($sql);
        if (!$resultado) {
            return false;
        }
        while ($status = $this->fetch_object($resultado)) {
            $nome_tabela = $status->Name;
            $comentario = $status->Comment;
            $pos = strpos($comentario, ';');
            if ($pos !== false) {
                $comentario = substr($comentario, 0, $pos);
            }
            $tamanho = $status->Data_length;
    ...
    //
        ...
        while ($campo = $this->fetch_object($resultado)) {
            ...
            if (isset($relacoes[$campo->nome])) {
                ...
                $campo->tabela_ref = null;
                $campo->chave_ref  = null;
            }
    ...
    //
        ...
        $sql_servidor = $this->delimitar_valor($this->servidor, 'string');
