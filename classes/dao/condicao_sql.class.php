...
final class condicao_sql {
    ...
    //   - String funcao1              // Funcao aplicada sobre o primeiro operando (dia, mes, ano, hora, minuto, segundo, diaano, diasemana, data, horario) (padrao vazio)
    //   - String funcao2              // Funcao aplicada sobre o segundo operando (dia, mes, ano, hora, minuto, segundo, diaano, diasemana, data, horario) (padro vazio)
    ...
    public function __toString() {
        ...
        case CONDICAO_SQL_SIMPLES:
            ...
            if (is_null($this->valores['operando2'])) {
                ...
                case '=':
                    return "{$classe}::montar('{$operando1}', '=', null{$id})";
                case '<>':
                    return "{$classe}::montar('{$operando1}', '&lt;&gt;', null{$id})";
                }
            } else {
                ...
                $flags = empty($vt_flags) ? '' : ', array('.implode(', ', $vt_flags).')';

                return "{$classe}::montar('{$operando1}', '{$operador}', {$operando2}{$flags}{$id})";
            }
            break;
        case CONDICAO_SQL_COMPOSTA:
            $id = $id ? ", '{$id}'" : '';
            ...
            if ($operador == 'AND') {
                return "{$classe}::sql_and({$vetor}{$id})";
            } elseif ($operador == 'OR') {
                return "{$classe}::sql_or({$vetor}{$id})";
            } else {
                return "{$classe}::montar_composta({$vetor}, {$operador}{$id})";
            }
            break;
        case CONDICAO_SQL_UNITARIA:
            $operador = $this->valores['operador'];
        ...
        case CONDICAO_SQL_AGRUPAMENTO:
            ...
            $vetor = "array(\n".implode(",\n", array_map('strval', $this->valores['vetor'])).")\n";
            if ($operador == 'UNION') {
                return "{$classe}::sql_union({$vetor}{$id})";
            }
            break;
        }
        return false;
...
/// @ METODOS FACTORY (CRIACAO DINAMICA DE OBJETOS)
    ...
    //     Int tipo1: indica o que e' o primeiro operando (padrao CONDICAO_SQL_TIPO_ATRIBUTO) veja constantes da classe
    //     Int tipo2: indica o que e' o segundo operando (padrao CONDICAO_SQL_TIPO_VALOR) veja constantes da classe
    //     String funcao1: indica a funcao aplicada sobre o primeiro operando (dia, mes, ano, hora, minuto, segundo, diaano, diasemana, data, horario)
    //     String funcao2: indica a funcao aplicada sobre o segundo operando (dia, mes, ano, hora, minuto, segundo, diaano, diasemana, data, horario)
    //
    ...
    //
    //     Cria uma condicao composta a partir dos valores das chaves/valores de um vetor
    //     utilizando um determinado operador simples (por padrao, a igualdade)
    //
    static public function montar_vetor($vetor, $operador_composto = 'AND', $operador_simples = '=', $id = '') {
    // Array[String => String] $vetor: vetor de chaves (campos da entidade) e valores (valor do campo)
    // String $operador_composto: um dos operadores de condicoes compostas
    // String $operador_simples: um dos operadores de condicoes simples
    // String $id: identificador unico da condicao
    //
        $vt_condicoes = array();
        foreach ($vetor as $chave => $valor) {
            $vt_condicoes[] = self::montar($chave, $operador_simples, $valor);
        }
        $condicoes = self::montar_composta($vt_condicoes, $operador_composto, $id);
        return $condicoes;
    }


    //
    //     Monta uma condicao composta
    //
    static public function montar_composta($vt_condicoes, $operador_composto, $id = '') {
    // Array[condicao_sql] $vt_condicoes: vetor de condicoes a serem agrupadas
    // String $operador_composto: operador composto AND ou OR
    // String $id: identificador unico da condicao
    //
        $quantidade_condicoes = count($vt_condicoes);
        switch ($quantidade_condicoes) {
        case 0:
            $condicao = self::vazia($id);
            break;
        case 1:
        ...
        default:
            ...
            $condicao->operador = $operador_composto;
    ...
    // String $id: identificador unico da condicao
    //
        return self::montar_composta($vt_condicoes, 'AND', $id);
    }
    ...
    // String $id: identificador unico da condicao
    //
        return self::montar_composta($vt_condicoes, 'OR', $id);
    }


    //
    //     Cria uma nova condicao unitaria
    //
    static public function montar_unitaria($condicao_original, $operador_unitario, $id = '') {
    // condicao_sql $condicao_original: condicao a ser negada
    // String $operador_unitario: operador unitario
    // String $id: identificador unico da condicao
    //
        $condicao = new self(CONDICAO_SQL_UNITARIA, $id);
        $condicao->operador = $operador_unitario;
    ...
    // String $id: identificador unico da condicao
    //
        return self::montar_unitaria($condicao_original, 'NOT', $id);
    }


    //
    //     Cria uma nova condicao de agrupamento
    //
    static public function montar_agrupamento($vt_condicoes, $operador_agrupamento, $id = '') {
    // Array[condicao_sql] $vt_condicoes: vetor de condicoes a serem agrupadas
    // String $operador_agrupamento: operador de agrupamento
    // String $id: identificador unico da condicao
    //
        $quantidade_condicoes = count($vt_condicoes);
        switch ($quantidade_condicoes) {
        ...
        default:
            ...
            $condicao->operador = $operador_agrupamento;
            $condicao->vetor    = $vt_condicoes;
            break;
        }
        if ($condicao->possui_erros()) {
    ...
    // String $id: identificador unico da condicao
    //
        return self::montar_agrupamento($vt_condicoes, 'UNION', $id);
    }
    ...
    //
    //     Cria uma condicao BETWEEN (entre) para especificar um intervalo de valores desejados
    //
...
/// @ METODOS AUXILIARES
    ...
    //
    static private function filtrar_operador_simples($operador) {
    ...
    //
        ...
        trigger_error('Operador "'.util::exibir_var($operador).'" desconhecido', E_USER_WARNING);
        return false;
    }


    //
    //     Filtra o operador composto para padronizacao
    //
    static private function filtrar_operador_composto($operador) {
    // String $operador: codigo do operador composto
    //
        $operador = trim(strtoupper($operador));
        switch ($operador) {
        case 'AND':
        ...
        trigger_error('Operador composto "'.util::exibir_var($operador).'" desconhecido', E_USER_WARNING);
    }


    //
    //     Filtra o operador unitario para padronizacao
    //
    static private function filtrar_operador_unitario($operador) {
    // String $operador: codigo do operador composto
    //
        $operador = trim(strtoupper($operador));
        switch ($operador) {

        // Operadores unitarios oficiais
        case 'NOT':
            return $operador;
    ...
    //
    //     Filtra o operador de agrupamento para padronizacao
    //
    static private function filtrar_operador_agrupamento($operador) {
    // String $operador: codigo do operador composto
    //
        $operador = trim(strtoupper($operador));
        switch ($operador) {

        // Operadores de agrupamento oficiais
        case 'UNION':
            return $operador;
        }
        trigger_error('Operador composto "'.util::exibir_var($operador).'" desconhecido', E_USER_WARNING);
    ...
    //
        ...
        case 'operando2':
            ...
            return;
        case 'operador':
            $valor = self::filtrar_operador_simples($valor);
            if ($valor) {
                $this->valores[$atributo] = $valor;
            }
            return;
    ...
    //
        ...
        case 'id':
            ...
            return;
        case 'operador':
            $valor = self::filtrar_operador_composto($valor);
            if ($valor) {
                $this->valores[$atributo] = $valor;
            }
            return;
    ...
    //
        ...
        case 'id':
            ...
            return;
        case 'operador':
            $valor = self::filtrar_operador_unitario($valor);
            if ($valor) {
                $this->valores[$atributo] = $valor;
            }
            return;
    ...
    //
        ...
        case 'id':
            ...
            return;
        case 'operador':
            $valor = self::filtrar_operador_agrupamento($valor);
            if ($valor) {
                $this->valores[$atributo] = $valor;
            }
            return;
        ...
        case CONDICAO_SQL_COMPOSTA:
            ...
            if (!$r) {
                ...
                foreach ($this->valores['vetor'] as $condicao) {
                    if (!is_object($condicao) || !($condicao instanceof $classe)) {
                        $r = true;
                        trigger_error('O vetor de condicoes de uma condicao composta precisa ser formado apenas por condicoes. Uma das possicoes possui um elemento '.util::get_tipo($condicao), E_USER_ERROR);
                        break;
                    }
                    if ($condicao->possui_erros()) {
                        $r = true;
...
/// @ METODOS DO PARSER
    ...
    //
    private static function estruturar_tokens($tokens, $posicao, $total) {
    ...
    //
    private static function criar_condicao_token($tokens) {
    ...
    //
        ...
        do {
            ...
            if (is_array($token)) {
                ...
                $operador = strtoupper($token->valor);
                switch ($operador) {
                ...
                case '!':
                    $token = array_shift($tokens);
                    if (!is_array($token)) {
                        trigger_error('Token invalido: esperado um vetor na condicao unitaria', E_USER_ERROR);
                    }
                    $vt_condicoes[] = self::montar_unitaria(self::criar_condicao_token($token), $operador);
                    break;

                // Checar se e' um operador composto
                case 'AND':
                case 'OR':
                    $novo_operador_composto = strtoupper($token->valor);
                    if (!$operador_composto || $novo_operador_composto == $operador_composto) {
                        $operador_composto = $novo_operador_composto;
                    } else {
                        $condicao = self::montar_composta($vt_condicoes, $operador_composto);
                        $operador_composto = $novo_operador_composto;
                    }
                    break;

                // Checar se e' uma condicao simples
                default:
                    if (is_array($token)) {
                        $vt_condicoes[] = self::criar_condicao_token($token);
                    } else {
                        ...
                        case 'UNLIKE':
                            ...
                            if ($operando2->tipo == TOKEN_SQL_STRING) {
                                if (is_numeric($operando2->valor)) {
                                    ...
                                    $operando2->valor = $operando2->valor == 'true';
                                } else {
                        ...
                        // Checar condicao IS NULL ou IS NOT NULL
                        case 'IS':
                            $operando2 = array_shift($tokens);
        ...
        default:
            return self::montar_composta($vt_condicoes, $operador_composto);
        }
    }
