...
/// @ Metodos gerais
    ...
    //
        if ($arquivo) {
            ...
            if ($this->img) {
                ...
                $this->erros[] = 'Erro ao criar arquivo de imagem';
            }
        }
    }
    ...
    //     Carrega um resource
    //
    public function load_resource($img, $arquivo = 'imagem.jpg', $mimetype = 'image/jpeg') {
    // Resource $img: imagem
    // String $arquivo: nome do arquivo
    ...
    //
        ...
        static $vt_tamanhos = array();

        if (isset($vt_tamanhos[$src])) {
            return $vt_tamanhos[$src];
        }

        $def = false;

        // Imagem local
        if (strpos($src, $CFG->wwwroot) === 0) {
            $arq = str_replace($CFG->wwwroot, $CFG->dirroot, $src);
            if (is_file($arq)) {
                $def = getimagesize($arq);
            }
        }
        ...
        $vt_tamanhos[$src] = $def ? $def[3] : '';
        return $vt_tamanhos[$src];
    }


    //
    //     Retorna um vetor com a largura e a altura em pixels
    //
    static public function tamanho($src) {
    // String $src: caminho da imagem
    //
        $def = getimagesize($src);
        if ($def) {
            return array($def[0], $def[1]);
        }
        return array(0, 0);
    }
    ...
    //
        ...
        if (isset($_SESSION[__CLASS__][$this->arquivo])) {
            $ultima_mudanca = $_SESSION[$this->arquivo];
        } else {
            $ultima_mudanca = $_SERVER['REQUEST_TIME'];
            $_SESSION[__CLASS__][$this->arquivo] = $ultima_mudanca;
        }

        $opcoes_http = array(
            'arquivo' => basename($this->arquivo),
            'tempo_expira' => $cache,
            'compactacao' => true,
            'ultima_mudanca' => $ultima_mudanca
        );

        http::cabecalho($mimetype, $opcoes_http);
...
/// @ Metodos para modificacao da imagem
    ...
    //
    public function redimensionar($altura = false, $largura = false, $suave = true) {
    ...
    // Bool $suave: redimensionar com algoritmo suave
    //
        if (!$this->img) { return null; }
        ...
        if ($suave) {
            imagecopyresampled($img, $this->img, 0, 0, 0, 0, $tam_x, $tam_y, $imagem_x, $imagem_y);
        } else {
