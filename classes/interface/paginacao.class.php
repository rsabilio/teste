...
final class paginacao {
    ...
    //
        ...
        if ($this->link) {
            link::normalizar($this->link, array($this->nome, 'enviar'));
        }
    }
    ...
    //
        return str_replace(' ', '_', $modulo.'-'.$id_lista);
    }
    ...
    public function consultar_pagina() {
        ...
        } elseif (isset($_GET[urlencode($this->nome)])) {
            $this->mudou_pagina = true;
            $this->salvar_pagina((int)$_GET[urlencode($this->nome)]);
    ...
    //
        ...
        if (!is_null($objeto)) {
            if ($total_itens == 0) {
                ...
                $quantidade = texto::numero($total_itens).' '.(($total_itens != 1) ? $this->get_plural() : $this->get_singular());
            }
    ...
    //
        ...
        $resultado = $this->objeto->consultar_varios_iterador($condicoes, $campos_consulta, $ordem,
                                                     $this->itens_pagina,
    ...
    public static function seta_esquerda() {
        global $CFG;
        if ($CFG->agent->engine == 'gecko' && $CFG->utf8) {
            return '&#x21E6;';
        } elseif ($CFG->agent->engine == 'mshtml') {
            return '&#x25C4;';
        }
        return '&larr;';
    }
