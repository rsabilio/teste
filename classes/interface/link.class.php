...
final class link {
    ...
    //
        ...
        if ($dados_script) {
            ...
            $dados_script = arquivo::consultar_arquivo_modulo($arquivo, $modulo, array('descricao'));
            if ($dados_script->existe()) {
                $d = ($descricao === false) ? $dados_script->exibir('descricao') : texto::codificar($descricao);
                $link = "<span class=\"inativo\" title=\"Sem permiss&otilde;es para acessar esta op&ccedil;&atilde;o\">{$d}</span>\n";
            }
        }
    ...
    //
    static public function icone_modulo(&$usuario, $modulo, $arquivo, $icone, $descricao = '', $texto = '', $exibir_texto = false, $carregar = true, $foco = true, $ajax = true) {
    // usuario $usuario: usuario para quem e' apresentado o link
    // String $modulo: nome do modulo
    // String $arquivo: nome do arquivo
    ...
    //
        ...
        if ($dados_script) {
            ...
            return self::icone($l, $icone, $descricao, $texto, $exibir_texto, $carregar, $foco, $ajax);
        }
        return '';
    }
    ...
    //
    static public function icone($link, $icone, $descricao = '', $texto = '', $exibir_texto = false, $carregar = true, $foco = true, $ajax = true) {
    // String $link: link de destino
    // String $icone: endereco do icone
    // String $descricao: descricao do icone
    // String $texto: texto explicativo opcional
    ...
    //
        ...
        if (isset($tamanhos[$icone])) {
            $tamanho_html = $tamanhos[$icone];
        } else {
            switch ($CFG->cookies['tamanho_icones']) {
            case '0':
                $fator = 0.8; // 80% do tamanho original
                $tamanho = imagem::tamanho($icone);
                $tamanho_html = $tamanhos[$icone] = sprintf('width="%0.2f." height="%0.2f"', $tamanho[0] * $fator, $tamanho[1] * $fator);
                break;
            default:
            case '1':
                $tamanho_html = $tamanhos[$icone] = imagem::tamanho_html($icone);
                break;
            case '2':
        ...
        if ($link) {
            ...
            return "<a href=\"{$link}\" title=\"{$descricao}\" {$onclick}>".
                   "<img src=\"{$icone}\" alt=\"{$descricao}\" {$tamanho_html} />".
                   (($exibir_texto) ? ' '.$texto : '').
                   "</a>";
        }
        return "<img src=\"$icone\" alt=\"{$texto}\" {$tamanho_html} />".
               (($exibir_texto) ? ' '.$texto : '');
    }
    ...
    //
        ...
        if (isset($dados['query']) && ($remover !== true)) {
            ...
            if (is_array($remover) && !empty($remover)) {
