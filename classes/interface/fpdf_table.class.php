...
class fpdf_table extends fpdf {
    ...
    //     Gets page width
    //
    public function GetPageWidth($pt = false) {
    // Bool $pt: width in poits
    //
        return $pt ? $this->wPt : $this->w;
    }
    ...
    //     Gets page height
    //
    public function GetPageHeight($pt = false) {
    // Bool $pt: height in poits
    //
        return $pt ? $this->hPt : $this->h;
    }
    ...
    //     Gets margins
    //
    public function GetMargin($pt = false) {
    // Bool $pt: unit in poits
    //
        if ($pt) {
            return array(
                't' => $this->tMargin * $this->k,
                'r' => $this->rMargin * $this->k,
                'b' => $this->bMargin * $this->k,
                'l' => $this->lMargin * $this->k
            );
        }
        return array(
            't' => $this->tMargin,
...
//TODO: implementar uma forma de fazer isso sem a extensao DOM
        ...
        for ($l = 0; $l < $lines; $l++) {
            $tr = $table->tr[$l];
            if ((double)$tr['height'] + ((double)2 * (double)$table['border']) > (double)$page_height) {
    ...
    //
        ...
        for ($c = 0; $c < $count_cells; $c++) {
            ...
            // Split text
            $this->SplitCell($table, simplexml_import_dom($cell), $part1, $part2);
    ...
    //
    private function SplitCell($table, $cell, &$part1, &$part2) {
    // SimpleXMLElement $table: table
    ...
    //
        ...
        $page_height = (double)$this->h - ((double)$this->bMargin + (double)$this->tMargin + (double)2 * (double)$table['border']);
        ...
        for ($i = count($vt_split_pos) - 1; $i >= 0; $i--) {
            ...
            $sub = (double)((double)2 * ((double)$border + (double)$padding));
    ...
    // SimpleXMLElement $table: table to be updated
    //
        $max_y = (double)$this->h - ((double)$this->bMargin + (double)$table['border']);
    ...
    //
        ...
        foreach ($table->tr as $tr) {
            ...
            if (DEBUG_FPDF_TABLE) {
                ...
                $this->Rect(0, $this->y, $this->w, (double)$tr['height']);
            }
...
//self::DebugTable($table);
    ...
    //
    public function getK($unit = null) {
    ...
    //
        ...
        $this->_out(sprintf('q %.2F 0 0 %.2F %.2F %.2F cm /I%d Do Q',$info['w'],$info['h'],$x*$this->k,($this->h-($y+$h))*$this->k,$info['i']));
